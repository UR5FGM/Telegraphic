...
def synth(freq, angles):
    ...
    gen = tone(freq)
    loop = list(
        itertools.islice(gen, (FRAME_RATE / freq) * (angles[1] / (2.0 * math.pi)))
    )[int((FRAME_RATE / freq) * (angles[0] / (2.0 * math.pi))) :]
    while True:
        for sample in loop:
            yield sample
...
def sample(generator, min=-1, max=1, width=SAMPLE_WIDTH):
    ...
    return (
        struct.pack("h", int((sample - min) * scale) - 2 ** (width * 8 - 1))
...
if __name__ == "__main__":
    b1 = beep(440, seconds=0.1)
    b2 = beep(880, seconds=0.1)
    # bb = list(b)
    # play(b2)
